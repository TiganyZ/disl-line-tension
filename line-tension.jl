...
function write_line_energies_of_images(d,X)
    for (i,x) in enumerate(X)
        write_line_energy_of_image(d, x, i)
    end
end
...
function get_kink_images(l1, l2, l3, offset, ip1, ip2, N; simple=false)
    ...
    # 120 length vectors
    d0 = SVector{2*(l1+l2+l3)}( vcat(disl0...) )
    d1 = SVector{2*(l1+l2+l3)}( vcat(disl1...) )
    dm = SVector{2*(l1+l2+l3)}( vcat(dislm...) )
